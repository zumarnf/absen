...
import type { CourseFormData } from "@/features/courses/schema";
import {
  Card,
  CardContent,
  CardDescription,
...
import { useUserMyCoursePage } from "@/features/courses/hooks/use-user-my-course-page";
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-12 animate-fade-in-up">
              ...
              <Button onClick={handleStartEditing} className="rounded-lg">
            ...
            <div className="space-y-4">
              ...
              {groupedCourses.map((course, index) => (
                <div
                  key={index}
                  className={`p-4 border rounded-lg card-hover animate-fade-in-up delay-${index + 1}`}
                >
                  ...
                  <div className="space-y-2">
                    {course.schedules.map((schedule, scheduleIndex) => (
                      <div
                        key={scheduleIndex}
                        className="flex items-center gap-4 text-sm text-muted-foreground bg-muted/40 p-2.5 rounded-lg"
                      >
    ...
    <div className="space-y-3">
      ...
      {fields.map((scheduleField, scheduleIndex) => (
        <div
          ...
          className="grid gap-3 md:grid-cols-4 items-end p-3 bg-card border rounded-lg"
        >
          <FormField
            control={control}
          ...
          <div>
            {fields.length > 1 && (
              <Button
                ...
                variant="ghost"
                size="sm"
                className="rounded-lg text-destructive hover:bg-destructive/10 hover:text-destructive w-full transition-colors"
