...
import { formatDateWithDay, getUserInfo } from "@/lib/helpers";
import { Pagination } from "@/components/shared/pagination";
import { ShiftPosBadge } from "@/components/shared/shift-pos-badge";
import { EmptyState } from "@/components/shared/empty-state";
import { TableSkeleton } from "@/components/shared/table-skeleton";
import { useAdminAttendancePage } from "@/hooks/pages/use-admin-attendance-page";
import dynamic from "next/dynamic";

const ExportPdfDialog = dynamic(
  () =>
    import("@/components/admin/export-pdf-dialog").then(
        ...
        <CardContent>
          ...
          <div className="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
            ...
            <div className="flex items-center gap-3">
              <Select value={selectedUserId} onValueChange={handleUserFilter}>
                <SelectTrigger className="w-[280px] bg-white rounded-xl">
          ...
          {selectedUser && (
            <div className="mb-4 flex items-center gap-2 animate-fade-in-up">
              ...
              <span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-solid-surface text-solid-dark">
                {selectedUser.nama} (@{selectedUser.username})
                <button
                  onClick={clearFilter}
                  className="ml-2 hover:text-solid-primary transition-colors"
                >
                  <X className="h-3 w-3" />
                </button>
              </span>
